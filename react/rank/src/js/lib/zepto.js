...
(function($) {
  function detect(ua, platform) {
    ...
    if (iphone && !ipod)
      (os.ios = (os.iphone = true)), (os.version = iphone[2].replace(
        /_/g,
        '.'
      ));
    if (ipad)
    ...
    if (ipod)
      (os.ios = (os.ipod = true)), (os.version = ipod[3]
        ? ipod[3].replace(/_/g, '.')
        : null);
    ...
    if (typeof properties == 'string') {
      ...
      for (key in properties)
        if (supportedTransforms.test(key))
          ...
          (cssValues[key] = properties[key]), cssProperties.push(
            dasherize(key)
          );
    ...
    return process(selector, function(sel, filter, arg) {
      try {
        ...
        if (!sel && filter)
          ...
          (taggedParent = $(node).addClass(classTag)), (sel = '.' +
            classTag +
            ' ' +
            sel);
  ...
  $(document).ready(function() {
    ...
    $(document)
      ...
      .on('touchstart MSPointerDown pointerdown', function(e) {
        if (
          (_isPointerType = isPointerEventType(e, 'down')) && !isPrimaryTouch(e)
        )
          return;
      ...
      .on('touchmove MSPointerMove pointermove', function(e) {
        if (
          (_isPointerType = isPointerEventType(e, 'move')) && !isPrimaryTouch(e)
        )
          return;
        firstTouch = _isPointerType ? e : e.touches[0];
        cancelLongTap();
      ...
      .on('touchend MSPointerUp pointerup', function(e) {
        if (
          (_isPointerType = isPointerEventType(e, 'up')) && !isPrimaryTouch(e)
        )
          return;
        cancelLongTap();
