...
class App extends Component {
  ...
  // 点击搜索按钮
  handleSearch(e){
    window.location = "/search.html?k="+ encodeURIComponent(this.state.searchKey) + "&s=" + encodeURIComponent(this.state.searchValue);
  }
  // 搜索框文本输入
  handleInputChange(e){
    this.setState({
  ...
  componentWillMount () {
    ...
    render(){
      ...
      if (desc) {
          ...
          descHTML.push(<p key="p2" className="desc">描述：{desc.Note.split(/\n/)}</p>)
      ...
      for (let i = 0; i < select.length; i++) {
        children.push(<Option key={i.toString(36) + i} value={select[i].value}>{select[i].text}</Option>);
      }
        ...
        <div>
          <header className="cf">
            ...
            <div className="search">
              ...
              <InputGroup className="ant-search-input" style={{ width: 200,float:'left' }}>
              <Input {...this.props} value={this.state.searchValue} onChange={this.handleInputChange.bind(this)}/>
                <div className="ant-input-group-wrap">
                  <Button className="ant-search-btn" onClick={this.handleSearch.bind(this)}>
                    <Icon type="search" />
                  </Button>
                </div>
              </InputGroup>
          ...
          <div className="wrapper cf">
            <div className="aside-container">
              <Menu style={ menuStyle } mode="inline" defaultOpenKeys={['sub1']} onClick={this.handleClick.bind(this)}>
                <SubMenu key="sub1" title={<span><Icon type="setting" /><span>搜房帮</span></span>}>
                {
                  menuData && menuData.map(item => <Menu.Item key={item.ID}>{item.Name}</Menu.Item>)
                }
                </SubMenu>
              </Menu>
            </div>
              ...
              {descHTML}
              {
                TableArr && TableArr.map((item,index) =>
                  <div key={index.toString(36)+index}>
                    <h4>{item.tab}</h4>
                    <Table key={index} rowKey={() => item.dataSource.ID} size="middle" columns={item.columns} dataSource={item.dataSource} pagination={false} bordered/>
                  </div>
                )
              }
              <div className={this.state.loading ? "loading":""}><Spin spining={this.state.loading}/></div>
            </div>
          </div>
        </div>
      )
