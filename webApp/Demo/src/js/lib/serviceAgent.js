    ...
    module.exports = {
        ...
        request: function (action, options, loading) {
            ...
            for (var i = 0; i < options.length; i++) {
                jsonArr.push({ 'url': options[i].url, 'args': this.buildArgs(options[i].data) });
            }
            alert(JSON.stringify(jsonArr));
            //执行调用
            document.addEventListener("deviceready", function () {
                if (loading) cordova.exec(null, null, "Notification", "activityStart", ["", loading]);
                cordova.exec(function (results) { //递归处理success后的回调
                    for (var i = 0; i < results.length; i++) {
                        if (results[i].isSuccess) {
                            if (options[i].success) { options[i].success.call(options[i], results[i].value, results); }
                        } else {
                            if (options[i].error) { options[i].error.call(options[i], results[i].value, results); }
                        }
                    }
                }, function (e) {
                    alert('调用失败:'+JSON.stringify(e));
                }, this.pluginClass, action, jsonArr);
                if (loading) Cordova.exec(null, null, 'Notification', 'activityStop', []);
            }, false);
        },
        /**
		 * 接口中转
